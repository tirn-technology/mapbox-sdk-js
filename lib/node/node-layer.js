...
function createRequestStreams(request) {
  ...
  if (global.myLogger && myLogger.info instanceof Function) {
    myLogger.info('Url: %s', url, {
      myId: request.myId,
      label: 'NodeClient.sendRequest'
    });
    myLogger.info('Headers: %s', gotOptions.headers, {
      myId: request.myId,
      label: 'NodeClient.sendRequest'
    });
    myLogger.info('Body: %s', gotOptions.body, {
      myId: request.myId,
      label: 'NodeClient.sendRequest'
    });
  }
      ...
      gotStream.on('end', function() {
        ...
        try {
          ...
          if (global.myLogger && myLogger.info instanceof Function) {
            myLogger.info(
              'Response headers: %s',
              JSON.stringify(httpsResponse.headers),
              { myId: request.myId, label: 'NodeClient.sendRequest' }
            );
            myLogger.info('Response body: %s', body, {
              myId: request.myId,
              label: 'NodeClient.sendRequest'
            });
          }
